...
```{r, echo=FALSE, warning=FALSE, include=FALSE}
#data
county_fairfax <- read.csv("~/Documents/R/git/cost-living-website/code/web/data_estimation/fairfax_2022_food_insecurity_rev.csv") %>% select(-c(moe, PUMA5CE_20))
...
#join data
county <- rbind(county_fairfax, county_arl, county_alexandria, county_prince_will, county_loudoun, county_frederick, county_montgomery, county_princegeorge, county_wash_dc)

#data
county_tract_sum <- county %>% group_by(GEOID=geoid) %>% summarise(housing_cost_avr = weighted.mean(housing_cost, num_hh))
#tracts geometry
tracts_county_va <- tracts(state = "VA", county = c("Fairfax County", "Arlington County", "Alexandria City", "Prince William County", "Loudoun County"), cb = FALSE, year = 2020) %>% select(STATEFP, COUNTYFP, TRACTCE, GEOID, NAME, NAMELSAD, geometry)

tracts_county_md <- tracts(state = "MD", county = c("Frederick County", "Montgomery County", "Prince George's County"), cb = FALSE, year = 2020) %>% select(STATEFP, COUNTYFP, TRACTCE, GEOID, NAME, NAMELSAD, geometry)
